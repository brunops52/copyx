...
const ShowRelations = (user:ShowRelationProps) => {
    ...
    useEffect(() => {
        ...
        const fetchIsFollowing = async () => {
          try {
            const response = await api.get<IsFollowingResponse>(`users/${user.user.id}/check-following/`);
        ...
        <div className="flex w-full py-5 px-12 gap-4 bg-black/70 border-b-1 border-neutral-700 items-center">
            ...
            {selfUser?.id != user.user.id ? (
                <>
                    {isFollowing ? (
                        <>
                            <button onClick={handleFollowSubmit} className="bg-black rounded-full border-1 border-neutral-500 py-1.5 px-4 ml-auto cursor-pointer">Seguindo</button>
                        </>
                    ):(
                        <>
                            <button onClick={handleFollowSubmit} className="bg-white rounded-full text-black py-1.5 px-4 ml-auto cursor-pointer">Seguir</button>
                        </>
                    )}  
                </>
            ):(<></>)}
